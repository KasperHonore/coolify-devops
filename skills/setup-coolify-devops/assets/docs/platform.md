...
| Coolify's application healthcheck **execs `curl`/`wget` inside the container** | A repo's own `-slim` Dockerfile image with neither is rolled back as unhealthy while the app runs fine; a finding for the builder, not a fix from here. Railpack images get both from Coolify (`RAILPACK_DEPLOY_APT_PACKAGES`). |
